...
public class MainGUI implements Observer {
	...
	private void initialize() {
		...
		TableRowSorter<TableModel> sorter = new TableRowSorter<>(fileNameList.getModel());
		fileNameList.setRowSorter(sorter);
		...
		fileNameList.getRowSorter().addRowSorterListener(new RowSorterListener() {
			...
			public void sorterChanged(RowSorterEvent e) {
				if (e.getType() == RowSorterEvent.Type.SORT_ORDER_CHANGED
						&& !fileNameList.getRowSorter().getSortKeys().isEmpty()) {
					fileList.sort(new Comparator<FileData>() {
						public int compare(FileData f1, FileData f2) {
							String t1 = f1.getChangedName();
							String t2 = f2.getChangedName();
