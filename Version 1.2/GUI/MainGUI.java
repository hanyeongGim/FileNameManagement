...
public class MainGUI {
	...
	private void initialize() {
		...
		fileList.add(new FileData("aa.txt"));
		addRow("aa.txt", "aa.txt");
	...
	private void addRow(String before, String after) {
		// 같은 내용이 있을경우
		/*
		 * for (int a = 0; a < modelJTable.getRowCount(); a++) { //Table명, 메뉴
		 * 이름, 계산 시간이 모두 일치할 경우 추가 if ( ( ((String) modelJTable.getValueAt(a,
		 * 0)).compareTo(String.format("Table%d", num+1)) ==0 )&&(
		 * modelJTable.getValueAt(a, 1) == name )&&( modelJTable.getValueAt(a,
		 * 6) == time )) { amount = (int) modelJTable.getValueAt(a, 2); total =
		 * (int) modelJTable.getValueAt(a, 4);
		 * 
		 * amount++; total += price; disprice = (int) (total -
		 * (0.2)*total*nowTable.getCondition());
		 * 
		 * modelJTable.setValueAt(amount, a, 2); modelJTable.setValueAt(total,
		 * a, 4); modelJTable.setValueAt(disprice, a, 5); return;
		 * 
		 * } }
		 */
